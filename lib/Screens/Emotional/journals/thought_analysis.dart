...
class ECheckinController extends GetxController {
  ...
  @override
  void onInit() {
    name.text = 'ERJournalLevel4ThoughtAnalysis${formatTitelDate(DateTime.now()).trim()}';
  ...
  Future addECI([bool fromsave = false]) async {
    ...
    if (page1.currentState!.validate()) {
      ...
      if (!fromsave) {
        if (Get.find<ErController>().history.value.value == 91) {
  ...
  fetchData() {
    ...
    date.text = _root.cdate!;
    time.text = _root.time!;
    check.value = _root.precedent!;
    personControllers.clear();
    for (int i = 0; i < _root.persons!.length; i++) {
      personControllers.add(TextEditingController(text: _root.persons![i]));
    }
    placeControllers.clear();
    for (int i = 0; i < _root.places!.length; i++) {
      placeControllers.add(TextEditingController(text: _root.places![i]));
    }
    actionControllers.clear();
    for (int i = 0; i < _root.actions!.length; i++) {
      actionControllers.add(TextEditingController(text: _root.actions![i]));
    }
    behaviourControllers.clear();
    for (int i = 0; i < _root.behaviors!.length; i++) {
      behaviourControllers
          .add(TextEditingController(text: _root.behaviors![i]));
    }
    triggerControllers.clear();
    for (int i = 0; i < _root.triggers!.length; i++) {
      triggerControllers.add(TextEditingController(text: _root.triggers![i]));
    }
    consequenceControllers.clear();
...
class ECIPrevious extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: customAppbar(
        title: 'ER Journal Level 4 - Thought Analysis ',
        implyLeading: true,
        actions: [],
      ),
      body: StreamBuilder(
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ListTile(
                  onTap: () => Get.to(() => const ThoughtAnalysis(),
...
class ThoughtAnalysis extends StatefulWidget {
  const ThoughtAnalysis({Key? key}) : super(key: key);

  @override
  _ThoughtAnalysisState createState() => _ThoughtAnalysisState();
}

class _ThoughtAnalysisState extends State<ThoughtAnalysis> {
