...
class EAsfController extends GetxController {
  ...
  @override
  void onInit() {
    name.text = 'ERJournalLevel4ThoughtAnalysisShortForm${formatTitelDate(DateTime.now()).trim()}';
  ...
  void addThought() {
    ...
    options.add(0);
    evidenceController.add(TextEditingController());
    againstController.add(TextEditingController());
    distortion.add(0);
  }

  final Rx<ETSfModel> _emotional = ETSfModel(
    id: generateId(),
  ...
  Future addEmotionalCheckin([bool fromsave = false]) async {
    ...
    if (formKey.currentState!.validate()) {
      ...
      if (!fromsave) {
        if (Get.find<ErController>().history.value.value == 92) {
  ...
  fetchData() {
    _emotional.value = Get.arguments[1] as ETSfModel;
...
class ThoughtAnalysisSf extends StatefulWidget {
  const ThoughtAnalysisSf({Key? key}) : super(key: key);

  @override
  State<ThoughtAnalysisSf> createState() => _ThoughtAnalysisSfState();
}

class _ThoughtAnalysisSfState extends State<ThoughtAnalysisSf> {
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    const TextWidget(
                      text:
                          'ER Journal Level 4 - \nThought Analysis Short Form (Practice)',
                ...
                JournalTop(
                  ...
                  drive: () => Get.off(() => const ETsfPrevious()),
                ),
      ...
      children: [
        ...
        Row(
          children: [
            CustomDropDownStruct(
              child: DropdownButton(
                value: _controller.options[index],
                onChanged: (val) {
                  setState(() {
                    _controller.options[index] = val!;
                  });
                },
                items: optionList
                    .map(
            ...
            SizedBox(
              ...
              child: CustomDropDownStruct(
                child: DropdownButton(
                  value: _controller.distortion[index],
                  onChanged: (val) {
                    setState(() {
                      _controller.distortion[index] = val!;
                    });
                  },
        ...
        Row(
          children: [
            SizedBox(
              width: Get.width * 0.453,
              child: TextFormField(
                maxLines: 8,
                controller: _controller.evidenceController[index],
            ...
            SizedBox(
              ...
              child: TextFormField(
                maxLines: 8,
                controller: _controller.againstController[index],
...
class ETsfPrevious extends StatelessWidget {
  const ETsfPrevious({Key? key}) : super(key: key);
        ...
        builder: (context, AsyncSnapshot snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              final ETSfModel model =
                  ETSfModel.fromMap(snapshot.data.docs[index]);
              return Card(
                elevation: 5.0,
                margin: const EdgeInsets.only(left: 15, right: 15, top: 10),
                child: ListTile(
                  onTap: () => Get.to(() => const ThoughtAnalysisSf(),
